...
func RegisterCallbacks(db *gorm.DB, opts ...Option) {
	...
	db.Callback().RowQuery().Before("ocgorm:row_query").Register("instrumentation:before_row_query", c.beforeQuery)
	db.Callback().RowQuery().After("ocgorm:row_query").Register("instrumentation:after_row_query", c.afterQuery)
	db.Callback().Create().Before("ocgorm:create").Register("instrumentation:before_create", c.beforeCreate)
	db.Callback().Create().After("ocgorm:create").Register("instrumentation:after_create", c.afterCreate)
	db.Callback().Query().Before("ocgorm:query").Register("instrumentation:before_query", c.beforeQuery)
	db.Callback().Query().After("ocgorm:query").Register("instrumentation:after_query", c.afterQuery)
	db.Callback().Update().Before("ocgorm:update").Register("instrumentation:before_update", c.beforeUpdate)
	db.Callback().Update().After("ocgorm:update").Register("instrumentation:after_update", c.afterUpdate)
	db.Callback().Delete().Before("ocgorm:delete").Register("instrumentation:before_delete", c.beforeDelete)
	db.Callback().Delete().After("ocgorm:delete").Register("instrumentation:after_delete", c.afterDelete)
}
...
func (c *callbacks) startTrace(ctx context.Context, scope *gorm.Scope, operation string) context.Context {
	...
	if parentSpan == nil {
		ctx, span = trace.StartSpan(
			context.Background(),
			fmt.Sprintf("ocgorm:%s", operation),
			trace.WithSpanKind(trace.SpanKindClient),
			trace.WithSampler(c.startOptions.Sampler),
		)
	} else {
		_, span = trace.StartSpan(ctx, fmt.Sprintf("ocgorm:%s", operation))
	}
	span.AddAttributes(trace.StringAttribute("ocgorm", operation))
